...
async function _getComment() {
    var comments = await Helper.fetchData(
        'select',
        `select comment.*, tbl_user.user_name AS username
        from comment
        inner join tbl_user on comment.user_id=tbl_user.user_id
    ...
    if (comments) {
        comments.forEach(item => {
            ...
            itemClone.querySelector(".input-group button").addEventListener('click', async () => {
                ...
                var user_id = await getIdUser(Helper.getLocalStorage('userName'))
...
async function getIdUser(username) {
    var data = await Helper.fetchData(
        'select',
...
class Helper {
    ...
    static async addComment(commentContent, user_id, parent_comment_id, baiviet_id) {
        if (!this.getLocalStorage('userRole') || !this.getLocalStorage('userName')) {
            alert('Vui lòng đăng nhập trước!')
            return
        }
        if (!commentContent || commentContent=="") {
            alert('Không thấy nội dung')
            return
        }
